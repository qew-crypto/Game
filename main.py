...
async def start_command(message: Message):
    ...
    if len(args) > 1 and args[1].startswith("ref_"):
        ...
        if str(referrer_id) in users and referrer_id != user_id:
            ...
            save_data(users)
            try:
                await bot.send_message(referrer_id, "🎉 Новый реферал! +1000🪙 +50💎")
            except:
                pass
...
async def click_handler(callback: CallbackQuery):
    ...
    if random.randint(1, 100) <= user.get("boosters", {}).get("crit_chance", 0):
        reward *= 2
...
async def donate_payment(callback: CallbackQuery):
    if callback.data in ["donate_menu", "donate_buy", "donate_money", "donate_energy", "donate_click", "donate_auto", "buy_vip"]:
        return
